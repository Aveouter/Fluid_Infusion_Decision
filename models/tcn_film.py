...
Outputs match your runner expectations:
...
- You can scale complexity via: embed_dim, n_transformer_layers, n_heads, tcn depth.
"""
...
def build_causal_mask(T: int, device: torch.device) -> torch.Tensor:
    ...
    # attn_mask shape: (T, T); True means masked.
...
# --------------------------
# Base blocks (same spirit as your previous file)
# --------------------------
...
class DualPathTCN(nn.Module):
    def __init__(self, c_in, c, layers_short=3, layers_long=5, k=3, causal=True, dropout=0.1):
        ...
        self.short = nn.ModuleList([
            TCNBlock(c if i>0 else c, c, k=k, d=2**i, causal=causal, dropout=dropout)
            for i in range(layers_short)
        ])
        self.long = nn.ModuleList([
            TCNBlock(c if i>0 else c, c, k=k, d=2**(i+1), causal=causal, dropout=dropout)
            for i in range(layers_long)
        ])
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        # x: (B,T,C)
        h = self.ln1(x)
        y, _ = self.attn(h, h, h, attn_mask=attn_mask)  # causal if mask is upper-triangular True
        x = x + self.drop(y)
        h = self.ln2(x)
        z = self.ff(h)
...
class TypeHead(nn.Module):
    ...
    def forward(self, x):  # (B,T,C) -> (B,T,C_cls)
        return self.net(x)
...
# --------------------------
# Full model: TCN → FiLM → (PE + Causal Transformer × L) → refine → heads
# --------------------------
class Baseline_TCN_FiLM_ZILN_Transformer(nn.Module):
    def __init__(self,
        ...
        self.tr_blocks = nn.ModuleList([
            TransformerBlock(embed_dim, n_heads, d_ff=ff_mult*embed_dim, dropout=dropout)
            for _ in range(n_transformer_layers)
        ])

        # lightweight refine
        self.refine = TCNBlock(embed_dim, embed_dim, k=3, d=1, causal=causal, dropout=dropout)
        ...
        # fusion gate between TCN and Transformer (optional, learnable)
        self.fuse_gate = nn.Sequential(
            nn.LayerNorm(embed_dim),
            nn.Linear(embed_dim, embed_dim), nn.GELU(),
            nn.Linear(embed_dim, 2), nn.Softmax(dim=-1)  # weights for [pre_tr, post_tr]
        )
    ...
    def forward(self, input_T: torch.Tensor, input_B: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # TCN encoding over full history
        ...
        # baseline fusion (FiLM on the prediction window features)
        b = self.base_proj(input_B)              # (B,E)
        h_film = self.sefilm(h_last, b)          # (B,P,E)

        # causal transformer over the P-window (can be larger if you pass longer windows)
        x = self.posenc(h_film)                  # (B,P,E)
        attn_mask = build_causal_mask(P, device) if self.causal else None
        pre_tr = x
        for blk in self.tr_blocks:
            x = blk(x, attn_mask=attn_mask)      # (B,P,E)
        post_tr = x

        # gated fusion of pre vs post transformer (stabilizes early training)
        ...
        # light refine for local smoothing
        z = self.refine(z)                       # (B,P,E)
...
# factory to match your builder signature

def build_model_tcn_film_ziln(
        input_dim_base: int,
        input_dim_temporal: int,
        embed_dim: int,
        num_heads: int,             # used by transformer
        hidden_dim: int,            # unused placeholder for compatibility
        num_classes: int,
        pred_length: int,
